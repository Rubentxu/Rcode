...
fn merge_configs(base: &RcodeConfig, overlay: &RcodeConfig) -> RcodeConfig {
    ...
    let overlay_json =
        serde_json::to_value(overlay).unwrap_or(serde_json::Value::Object(serde_json::Map::new()));
...
mod config_path_tests {
    ...
    fn test_find_highest_existing_path_returns_first_match() {
        ...
        let candidates = vec![base.join("a.txt"), base.join("b.txt"), base.join("c.txt")];
        std::fs::write(base.join("b.txt"), "test").unwrap();

        assert_eq!(
            find_highest_existing_path(&candidates),
            Some(base.join("b.txt"))
        );
    }
    ...
    fn test_find_highest_existing_path_returns_none_when_no_match() {
        ...
        let candidates = vec![base.join("a.txt"), base.join("b.txt")];
...
pub fn save_config(config: &RcodeConfig) -> Result<(), String> {
    ...
    if !config_without_secrets.extra.is_null()
        && config_without_secrets.extra != serde_json::json!({})
...
mod tests {
    ...
    async fn test_load_config_from_jsonc_file() {
        ...
        let config = load_config(Some(config_path), false, Some(temp.path().to_path_buf()))
            .await
            .unwrap();
    ...
    async fn test_cascade_merging() {
        ...
        let result = load_config(Some(global_path), false, Some(temp.path().to_path_buf()))
            .await
            .unwrap();
    ...
    async fn test_save_config_includes_lsp() {
        ...
        assert_eq!(
            rust_config.get("command").unwrap().as_str().unwrap(),
            "rust-analyzer"
        );
        assert_eq!(
            rust_config.get("args").unwrap().as_array().unwrap()[0]
                .as_str()
                .unwrap(),
            "--verbose"
        );
        assert_eq!(
            rust_config.get("cwd").unwrap().as_str().unwrap(),
            "/workspace"
        );
    }
