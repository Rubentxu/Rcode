...
use crate::error::{Result, OpenCodeError};
...
impl AgentLoader {
    ...
    async fn load_from_file(&self, path: &Path) -> Result<AgentDefinition> {
        let content = tokio::fs::read_to_string(path)
            .await
            .map_err(|e| OpenCodeError::Config(format!("Failed to read agent file {}: {}", path.display(), e)))?;
        
        let definition: AgentDefinition = serde_json::from_str(&content)
            .map_err(|e| OpenCodeError::Config(format!("Failed to parse agent JSON {}: {}", path.display(), e)))?;
